...
export function UpdateMenuItem({ id }) {
  ...
  useEffect(() => {
    if (menuItem) {
      const actualMenuItem = Array.isArray(menuItem) ? menuItem[0] : menuItem;
      let parsedImages = [];

      try {
        if (actualMenuItem.imageUrl) {
          const parsed = JSON.parse(actualMenuItem.imageUrl);
          parsedImages = Array.isArray(parsed) ? parsed : [actualMenuItem.imageUrl];
        }
      } catch (err) {
        parsedImages = [actualMenuItem.imageUrl];
      }

      form.reset({
        name: actualMenuItem.name || "",
        price: String(actualMenuItem.price || ""),
        category: actualMenuItem.category || "",
        description: actualMenuItem.description || "",
        status: actualMenuItem.status || "AVAILABLE",
      });

      setExistingImageUrls(parsedImages);
      setImagePreviews(parsedImages); // Show existing first
      setNewImageFiles([]);
  ...
  if (fetchError) {
    return (
      <div className="text-center p-8 text-red-500">
        Error loading menu item: {fetchError.message || "Unknown error."}
      </div>
    );
  }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormItem>
              ...
              <FormDescription>
                Upload new images or keep the existing ones. Removed images will not be saved.
              </FormDescription>
            </FormItem>
            ...
            <Button
              type="submit"
              disabled={isSubmitting || imagePreviews.length === 0 || (!isDirty && newImageFiles.length === 0)}
              className="mt-2"
            >
      ...
      <div className="w-full lg:w-1/3">
        ...
        <Card className="w-full overflow-hidden py-0">
          ...
          <CardHeader className="p-3">
            ...
            <CardDescription className="text-xs">{watchedValues.category}</CardDescription>
          </CardHeader>
